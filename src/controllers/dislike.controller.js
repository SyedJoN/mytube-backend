...
export const toggleDislike = (entity) => asyncHandler(async (req, res, next) => {
    const { id } = req.params;
    ...
    if (existingDislike) {
      ...
      return res.status(200).json(new ApiResponse(200, {}, `${entity} undisliked successfully!`));
    }
  
    // Otherwise, add a new dislike
    await Dislike.create(dislikeQuery);
    return res.status(200).json(new ApiResponse(200, {}, `${entity} disliked successfully!`));
  });
