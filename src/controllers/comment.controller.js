...
const getVideoComments = asyncHandler(async (req, res) => {
  ...
  const getLikesAndDislikes = async (commentId) => {
    ...
    const dislikedBy = await Dislike.find({comment: commentId}).select(
      "dislikedBy"
    );
  ...
  const commentLikesDislikes = await Promise.all(
    videoComments.map(async (comment) => {
      const {likesCount, likedBy, dislikedBy} = await getLikesAndDislikes(
        comment._id
      );
...
const updateComment = asyncHandler(async (req, res) => {
  ...
  if (!updatedComment) {
    throw new ApiError(
      403,
      "Unauthorized to edit this comment or comment not found!"
    );
  }
